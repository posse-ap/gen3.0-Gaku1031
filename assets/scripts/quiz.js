  ...
  allQuiz.forEach(quiz => {
    ...
    answers.forEach(answer => {
      answer.addEventListener('click', () => {
        ...
        //シャッフルしたquizArrayのselectedQuizでquizの番号を取得し、その配列の中でcorrectNumberと等しいものが定数correctNumber
        //correctNumberとselectedAnswerNumberが等しいものが定数isCorrect
        const correctNumber = quizArray[selectedQuiz].correctNumber
        const isCorrect = correctNumber === selectedAnswerNumber; 

        //回答欄にテキストやクラス名を付与
        //一番上で定義した配列のanswersの中でocrrectNumberと等しい答えをとってくる。
